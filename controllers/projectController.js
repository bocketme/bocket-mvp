...
let lastUpdate = require('./utils/lastUpdates');
let allNode = require('./utils/allNode');
...
function getLastUpdatesPage(workspaceId, userMail, title) {
    ...
    return new Promise((resolve, reject) => {
        Workspace.findById({_id: workspaceId})
            .catch((err) => {
                console.error(new Error("Error - MongoDB cannot search \n", err));
                return Promise.reject({code: 500, message: "Intern Error"});
            })
            .then(workspace => {
                if(!workspace){
                    console.error("Cannot Find The Workspace");
                    return Promise.reject({code: 404, message: "Workspace Not Found"});
                }

                Collections.workspace = workspace;

                return allNode(workspace.node_master._id);
            })
            .catch(err => {
                if (err.message && err.status)
                    return Promise.reject(err);

                console.error(new Error("Error - Cannot recuperate the number of node for a Workspace\n", err));
                return Promise.reject({code: 500, message: "Intern Error"});
            })

            .then(nodeWorkspace => {
                Counters.workspace = nodeWorkspace;
            ...
            .catch(err => {
                ...
                console.error(new Error("Error - Cannot recuperate the number of last Update for a Node\n", err));
                return Promise.reject({code: 500, message: "Intern Error"});
            })

            .then(nodeLastUpdate => {
                Counters.lastUpdate = nodeLastUpdate;

                return User.findOne({email: userMail});
            })
            .catch(err => {
            ...
            .then(user => {
                ...
                return Node.findById(Collections.workspace.node_master._id);
            })
            .catch(err => {
                if (err.message && err.status)
                    return Promise.reject(err);
